...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(0, 0, 0);
		text("City Marker", 75, 128);
		text("Land Quake", 75, 148);
		text("Ocean Quake", 75, 168);
		text("Shallow",75, 218);
		text("Intermediate",75,238);
		text("Deep",75,258);
		text("Size ~ Magnitude",40,193);
	}
	...
	private void printQuakes() 
	{
		...
		for(Marker cm : countryMarkers)
		{
			...
			for(Marker m : quakeMarkers)
			{
				...
				{
					if(m.getProperty("country").toString().equals(name))
						counter++;
				}
			}
			if(counter!=0)
